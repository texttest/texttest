...
class PlotKPIGroupsAndGeneratePage(apc.PlotKPIGroups):
    ...
    def __del__(self):
        ...
        for group in self.allGroups:
            ...
            plotRow = [ HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 0, 0, None))),
                        HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 1, 0, None)))]
            if self.plotMem:
                plotRow.append(HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 0, 1, None))))
    ...
    def setExtraOptions(self, optionGroup, group):
        ...
        optionGroup.setValue("p", self.getPlotName(group, self.onlyAverage, self.doMemPlot))
    ...
    def getPlotName(self, group, average, mem, fullPath = 1):
        if not mem:
