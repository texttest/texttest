...
class ApplicationFileGUI(FileViewGUI):
    ...
    def monitorEvents(self):
        guiutils.scriptEngine.monitorSignal("select application file", "row-activated", self.selection.get_tree_view())
...
class TestFileGUI(FileViewGUI):
    ...
    def monitorEvents(self):
        # Needed to ensure the recorder gets its handler in before the application events generated by this
        guiutils.scriptEngine.monitorSignal("select file", "row-activated", self.selection.get_tree_view())
