...
def runWindowManager(logDir, wmExecutable, displayName, extraArgs):
    ...
    logFileName = os.path.basename(os.path.normpath(wmExecutable)) + "." + displayName.lstrip(":") + "." + gethostname().split(".")[0] + ".diag"
    ...
    proc = subprocess.Popen(command, env=subEnv, stdout=open(logFilePath, "w"), stderr=subprocess.STDOUT, stdin=open(os.devnull))
