...
class PlotKPIGroupsAndGeneratePage(apc.PlotKPIGroups):
    def __init__(self, args = []):
        ...
        self.versions = [ "master", "12" ]
        self.groupTimes = {}
        for version in self.versions:
            self.groupTimes[version] = {}
    def __del__(self):
        self.onlyAverage = 0
        ...
        for group in self.allGroups:
            table.append(HTMLgen.TR() + [HTMLgen.TH("KPI group " + group, colspan = 3)])
            table.append(HTMLgen.TR() + [ HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 0, None))),
                                          ...
                                          HTMLgen.TD(HTMLgen.Image(self.getSpendTimeName(group, None)))])
        doc.append(table)
    ...
    def getPlotName(self, group, average, fullPath = 1):
        ...
        if fullPath:
            ...
            return plotName
    def getSpendTimeName(self, group, fullPath = 1):
        plotName = group + "_st" + ".png"
        if fullPath:
            return os.path.join(self.dir, plotName)
        else:
            return plotName
    ...
    def extraActionForTest(self, group, test):
        for version in self.versions:
            times = self.extractColgenData(test.app, test.makeFileName(test.app.getConfigValue("log_file"), version, temporary = 0))
            if not self.groupTimes.has_key(group):
                self.groupTimes[version][group] = times
            else:
                self.groupTimes[version][group] = map(lambda x,y:x+y, self.groupTimes[group], times)
    def extractColgenData(self, app, statusFile):
        ...
        tl.append(totTime - sum)
        return tl
    def generatePies(self):
        if not mplDefined:
            print "No matplotlib"
            return

        labels = 'Network', 'Generation', 'PAQS', 'Deadhead post', 'Conn. fix', 'OC->DH', 'Other'
        colours = ["red", "blue", "green", "gray", "yellow", "magenta", "black" ]
        for group in self.allGroups:
            figure(1, figsize=(2.5,5))
            clf()
            ind = arange(len(self.versions)) + 0.3  # the x locations for the groups
            width = 0.4     # the width of the bars
            yoff = array([0.0] * len(self.versions)) # the bottom values for stacked bar chart
            for index in range(len(labels)):
                data = []
                for version in self.versions:
                    data.append(self.groupTimes[version][group][index])
                bar(ind, data, width, bottom=yoff, color=colours[index])
                yoff = yoff + data
            xticks(ind + width*0.5, self.versions)
            savefig(self.getSpendTimeName(group))
            
##            figure(1, figsize=(2.5,5))
##            clf()
##            ax = axes([0.1, 0.1, 0.8, 0.4])
##            explode=(0.05, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05)
##            pie(self.groupTimes[""][group], explode=explode, labels=labels, autopct='%1.1f%%', shadow=True)
##            title("master")
##            ax = axes([0.1, 0.5, 0.8, 0.4])
##            pie(self.groupTimes["12"][group], explode=explode, labels=labels, autopct='%1.1f%%', shadow=True)
##            title("12")
##            savefig(self.getSpendTimeName(group))
