...
class PlotKPIGroupsAndGeneratePage(apc.PlotKPIGroups):
    ...
    def __del__(self):
        ...
        for group in self.allGroups:
            ...
            table.append(HTMLgen.TR() + [ HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 0))),
                                          HTMLgen.TD(HTMLgen.Image(self.getPlotName(group, 1)))])
        doc.append(table)
    ...
    def setExtraOptions(self, optionGroup, group):
        optionGroup.setValue("av", self.average)
        optionGroup.setValue("oav", self.average)
