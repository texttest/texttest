...
def findBugInfo(bugId, location, *args):
    ...
    try:
        ...
        message = "Failed to open URL '" + tracRequest + "': " + str(e) + \
                  ...
                  "points to the correct trac instance. The current value is '" + location + "'."
        return "BAD SCRIPT", message, False
    keys = reply[0].split('\t')
    values = ("".join(reply[1:])).split('\t')
    if len(keys) == 1:
        message = "Bug " + bugId + " could not be found in the trac instance at " + location + "."
...
if __name__ == "__main__":
    import sys
    print findBugInfo(sys.argv[1], sys.argv[2])[1]
