...
class InteractiveAction(plugins.Observable):
    ...
    def isTestDependent(self):
        return False
    def updateOptionGroup(self, test, state):
        pass
    ...
    def addOption(self, key, name, value = "", possibleValues = []):
        return self.optionGroup.addOption(key, name, value, possibleValues)
    def addSwitch(self, key, name, defaultValue = 0, options = []):
        return self.optionGroup.addSwitch(key, name, defaultValue, options)
...
class ViewFile(InteractiveTestAction):
    ...
    def addDifferenceSwitches(self):
        changed = False
        changed |= self.addSwitch("rdt", "Include Run-dependent Text", 0)
        changed |= self.addSwitch("nf", "Show differences where present", 1)
        return changed
    def updateOptionGroup(self, test, state):
        if test is self.currentTest:
            return self._updateOptionGroup(state)
        else:
            return False
    def _updateOptionGroup(self, state):
        changed = False
        if self.isActiveOnCurrent():
            followDefault = self.optionGroup.getSwitchValue("f", 1)
            changed |= self.optionGroup.removeSwitch("f")
            if not state.hasResults():
                changed |= self.optionGroup.removeSwitch("rdt")
                self.optionGroup.removeSwitch("nf")
            if state.hasResults():
                changed |= self.addDifferenceSwitches()
            if not state.isComplete():
                changed |= self.addSwitch("f", "Follow file rather than view it", followDefault)
        return changed
    def updateDefaults(self):
        if self.dynamic:
            self._updateOptionGroup(self.currentTest.state)
...
class RecomputeTest(InteractiveTestAction):
    ...
    def updateDefaults(self):
        ...
        if state.isComplete() and state.needsRecalculation():
            self.perform(autoGenerated=True)
    def getTitle(self):
        return "Progress"
    def getScriptTitle(self, tab):
